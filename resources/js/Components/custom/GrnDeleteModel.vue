          ...
          >
            ...
            >
              ...
              <p class="text-[15px] text-center text-gray-700">
                Are you sure you want to delete this Grn? This action cannot
                be undone.
              </p>
...
// };
  ...
  // Props for the modal
  const { open, selectedGrn } = defineProps({
    open: {
      type: Boolean,
      required: true,
    },
  ...
  const deleteItem = () => {
  if (!selectedGrn?.id) return;

  form.delete(route('grns.destroy', selectedGrn.id), {
    onSuccess: () => {
